...
```yaml
  createpullsecret:
    ...
    stage: deploy
    commands:
    - codefresh generate image-pull-secret --cluster [CLUSTER_NAME] --namespace --registry ACR
```
...
{% raw %}
  deploy:
    ...
    environment:
      ...
      - KUBE_CONTEXT=danAzure
      - CUSTOM_service.type=LoadBalancer
      - CUSTOM_deployment[0].track=release
      - CUSTOM_deployment[0].image.repository=r.cfcr.io/todaywasawesome/${{CF_REPO_OWNER}}/${{CF_REPO_NAME}}
