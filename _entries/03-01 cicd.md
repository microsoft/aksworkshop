...
> **Important** Make sure to change the value of the `imageRepository` variable from `uniqueacrname.azurecr.io/captureorder` to your Azure Container Registry details, for example `acr73505.azurecr.io/captureorder`.
...
- `imageRepository` refers to where the image is deployed, for example `acr73505.azurecr.io/captureorder`. You need to change this to your environment details.
- `tag` refers to the autogenerated build ID. This is used to tag the container images built and deployed.
...
variables:
  ...
  - name: imageRepository
    value: 'acr73505.azurecr.io/captureorder' # replace with your own image name to match the name in the deployment.yaml, for example uniqueacrname.azurecr.io/captureorder
...
> **Note** For this to work, you should make sure to change the value of the `imageRepository` in the pipeline to match your `deployment.yaml` file. For example: `uniqueacrname.azurecr.io/captureorder`.
